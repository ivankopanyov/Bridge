...
const NavBar: FC<Readonly<PropsWithChildren<NavBarProps>>> = ({
  title,
  icon,
  loading,
  search,
  ...
  return (
    <div>
      <AppBar
        className={`nav-bar ${!screenSize.isMobile && "nav-bar-desktop"}`}
      >
        <Toolbar>
          ...
          {drawer &&
            (screenSize.width > 700 || (!fixSearch && !showSearch)) && (
              <div className="nav-bar-button-right">
      ...
      <div className="nav-bar-page">
        {
          <Drawer anchor="right" open={showDrawer} onClose={onCloseDrawerClick}>
            {drawer}
          </Drawer>
        }
