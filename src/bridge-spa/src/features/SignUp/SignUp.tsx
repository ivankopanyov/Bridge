...
const SignUp: FC = () => {
  ...
  const disable = () =>
    !new RegExp(
      `^[ ]*[A-Za-z][A-Za-z1-9-_]{${NAME_MIN - 1},${NAME_MAX - 1}}[ ]*$`
    ).test(username) ||
  ...
  const usernameTitle = () => {
    ...
    const length =
      name.length >= NAME_MIN && name.length <= NAME_MAX ? ok : fail;
  ...
  const passwordTitle = () =>
    `Password (${
      password.length >= PASS_MIN && password.length <= PASS_MAX ? ok : fail
    } Длина в диапазоне от ${PASS_MIN} до ${PASS_MAX} символов.)`;

  const confirmPasswordTitle = () =>
    `Confirm password (${
      password === confirnPassword ? ok : fail
    } Пароли совпадают.)`;
