...
export const getTask = createAsyncThunk(
  "logList/getTask",
  async (id: string) => await api.get(`/logs/${id}`)
);

export const getLog = createAsyncThunk(
  "logList/getLog",
  async (id: { taskId: string; id: string }) =>
...
const logListSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      getTasks.fulfilled,
      (state, action: PayloadAction<LogInfo[]>) => {
        addLogs(state, action.payload);
      }
    );
    builder.addCase(getTasks.rejected, (state, action) => {
      if (action.error.code === "401") state.auth = false;
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<LogInfo[]>) => {
        ...
        const task = state.tasks.find(
          (t) => t.logs[0].taskId === logs[0].taskId
        );
        if (task) {
          let logSort = false;
          let sort = false;
    ...
    builder.addCase(getTask.rejected, (state, action) => {
      ...
      if (task) {
        ...
        if (action.error.code !== "401") task.error = action.error.message;
      }
    ...
    builder.addCase(getLog.pending, (state, action) => {
      ...
      const log = state.tasks
        .find((t) => t.logs[0].taskId === taskId)
        ?.logs?.find((l) => l.id === id);
      if (log) log.loading = true;
    });
    builder.addCase(
      getLog.fulfilled,
      (state, action: PayloadAction<LogData>) => {
        const logData = action.payload;
        if (logData.inputObjectJson)
          logData.inputObjectJson = JSON.stringify(
            JSON.parse(logData.inputObjectJson),
            null,
            4
          );

        const log = state.tasks
          .find((t) => t.logs[0].taskId === logData.taskId)
          ?.logs?.find((l) => l.id === logData.logId);
        if (log) {
          log.data = logData;
          log.loading = false;
          log.error = undefined;
    ...
    builder.addCase(getLog.rejected, (state, action) => {
      ...
      if (log) {
        ...
        if (action.error.code !== "401") log.error = action.error.message;
      }
...
export const { addLog, addLogRange, setLoading, setError, authorized } =
