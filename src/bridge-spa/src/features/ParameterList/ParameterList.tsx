...
export const ParameterList: FC<Readonly<ParameterListProps>> = ({
  ...
  const showButtons =
    parameters.booleanParameters.length > 0 ||
  ...
  const onSaveClick = async () => {
    const parameters = produce(modifiedParameters, (draftState) => {
      draftState.listParameters.forEach(
        (parameter) =>
          (parameter.value = parameter.value.filter((i) => i.trim() !== ""))
      );
      draftState.booleanMapParameters.forEach(
        (parameter) =>
          (parameter.value = parameter.value.filter((i) => i.key.trim() !== ""))
      );
      draftState.stringMapParameters.forEach(
        (parameter) =>
          (parameter.value = parameter.value.filter((i) => i.key.trim() !== ""))
      );
    });
  ...
  const onBooleanParameterChanged = (index: number, value: boolean) =>
    setModifiedParameters(
      produce(modifiedParameters, (draftState) => {
  ...
  const onListParameterItemChanged = (
    index: number,
    itemImdex: number,
    value: string
  ) =>
  ...
  const addBooleanMapParameterItem = (index: number) =>
    setModifiedParameters(
      produce(modifiedParameters, (draftState) => {
        draftState.booleanMapParameters[index].value.push({
          key: "",
          value: false,
        });
      })
  ...
  const onKeyBooleanMapParameterItemChanged = (
    index: number,
    itemIndex: number,
    value: string
  ) =>
  ...
  const onValueBooleanMapParameterItemChanged = (
    index: number,
    itemIndex: number,
    value: boolean
  ) =>
  ...
  const addStringMapParameterItem = (index: number) =>
    setModifiedParameters(
      produce(modifiedParameters, (draftState) => {
        draftState.stringMapParameters[index].value.push({
          key: "",
          value: "",
        });
      })
  ...
  const onKeyStringMapParameterItemChanged = (
    index: number,
    itemIndex: number,
    value: string
  ) =>
  ...
  const onValueStringMapParameterItemChanged = (
    index: number,
    itemIndex: number,
    value: string
  ) =>
  ...
  return (
    <div>
      {showButtons &&
        (editMode ? (
          <div className="parameter-list-button-container">
            <Button
              className="parameter-list-button"
              onClick={onSaveClick}
              disabled={disable}
            >
            ...
            <Button
              className="parameter-list-button"
              onClick={() => setModifiedParameters(parameters)}
              disabled={disable}
            >
      ...
      {(error || stackTrace) && <Error error={error} stackTrace={stackTrace} />}
      {(editMode ? modifiedParameters : parameters).booleanParameters.length >
        0 && (
        <div className="parameter-list-bool-params-container">
          {(editMode ? modifiedParameters : parameters).booleanParameters.map(
            (parameter, index) => (
      ...
      {(editMode ? modifiedParameters : parameters).stringParameters.map(
        (parameter, index) => (
          <div key={index}>
            <ParameterHeader name={parameter.name} />
            <TextField
      ...
      {(editMode ? modifiedParameters : parameters).listParameters.map(
        (parameter, index) => (
          <EnumerableParameter
            key={index}
            ...
            {parameter.value.map((item, i) => (
              ...
              >
                <TextField
                  value={item}
                  onChange={(e) =>
                    onListParameterItemChanged(index, i, e.target.value)
                  }
                  disabled={disabled}
                  fullWidth
                  multiline
      ...
      {(editMode ? modifiedParameters : parameters).booleanMapParameters.map(
        (parameter, index) => (
          <EnumerableParameter
            key={index}
            title={parameter.name}
              ...
              >
                <div className="parameter-list-boolean-parameter">
                  <Checkbox
                    value={item.value}
                    setValue={(value) =>
                      onValueBooleanMapParameterItemChanged(index, i, value)
                    }
                    disabled={disabled}
                    large
                  />
                  <TextField
                    value={item.key}
                    onChange={(e) =>
                      onKeyBooleanMapParameterItemChanged(
                        index,
                        i,
                        e.target.value
                      )
      ...
      {(editMode ? modifiedParameters : parameters).stringMapParameters.map(
        (parameter, index) => (
          <EnumerableParameter
            key={index}
            title={parameter.name}
            ...
            {parameter.value.map((item, i) => (
              ...
              >
                <TextField
                  ...
                  value={item.key}
                  onChange={(e) =>
                    onKeyStringMapParameterItemChanged(index, i, e.target.value)
                  }
                  disabled={disabled}
                  fullWidth
                  multiline
                ...
                <TextField
                  ...
                  value={item.value}
                  onChange={(e) =>
                    onValueStringMapParameterItemChanged(
                      index,
                      i,
                      e.target.value
                    )
