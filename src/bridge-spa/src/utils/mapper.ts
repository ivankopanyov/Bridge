...
export const object = (parameters: Parameters) =>
  Object.fromEntries([
    ...
    ...parameters.booleanMapParameters.map((p) => [
      p.name,
      Object.fromEntries(p.value.map((i) => [i.key, i.value])),
    ]),
    ...parameters.stringMapParameters.map((p) => [
      p.name,
      Object.fromEntries(p.value.map((i) => [i.key, i.value])),
    ]),
  ]);
      ...
      case "map":
        ...
        if (mapValues.length > 0 && getType(mapValues[0]) === "boolean") {
          parameters.booleanMapParameters.push({
            name: key,
            value: Object.keys(values[index]).map((k, i) => {
              return {
                key: k,
...
const getType = (value: any): "boolean" | "string" | "array" | "map" => {
  ...
  if (type === "object") return Array.isArray(value) ? "array" : "map";
  return "string";
};
